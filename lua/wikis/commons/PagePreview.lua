...
-- module-level collector; persists across #invoke calls within a single parse
PagePreview._collected = {}
PagePreview._count = 0

---clears the collector (test-only)
...
function PagePreview.register(pageName)
	...
	local key = PagePreview.key(pageName)
	if PagePreview._collected[key] or PagePreview._count >= MAX_ENTITIES then
		return
	end
	PagePreview._collected[key] = true
	PagePreview._count = PagePreview._count + 1
end
...
function PagePreview.collectCards()
	local keys = {}
	for key in pairs(PagePreview._collected) do
