...
function LegacyMatchList.matchMaps(frame)
	...
	local mapData = LegacyMatchList.handleMaps(details)
	Table.deepMergeInto(processedArgs, details, mapData)
...
function LegacyMatchList.handleMaps(processedArgs)
	local maps = Array.mapIndexes(function(mapIndex)
		return Logic.nilIfEmpty{
			winner = Table.extract(processedArgs, 'map' .. mapIndex .. 'win'),
			vod = Table.extract(processedArgs, 'vodgame' .. mapIndex),
			map = Table.extract(processedArgs, 'map' .. mapIndex),
			overtime = Table.extract(processedArgs, 'ot' .. mapIndex),
			score1 = Table.extract(processedArgs, 'map' .. mapIndex .. 't1score'),
			score2 = Table.extract(processedArgs, 'map' .. mapIndex .. 't2score'),
		}
	end)
