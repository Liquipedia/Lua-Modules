...
function BracketDisplay.NodeLowerConnectors(props)
	...
	for _, lowerEdge in ipairs(bracketData.lowerEdges) do
		...
		local leftTop = layout.lowerNodeMarginTop + heightSums[lowerEdge.lowerMatchIx] + lowerLayout.mid
		local rightTop = layout.matchMarginTop + ((lowerEdge.opponentIx - 1) + 0.5) * config.opponentHeight
		local jointLeft = (config.roundHorizontalMargin - 2) * jointIxs[lowerEdge.opponentIx] / (jointCount + 1)
