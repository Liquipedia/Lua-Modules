...
function BracketDisplay.alignMatchWithLowerNodes(match, lowerLayouts, heightSums, opponentHeight)
	...
	if #lowerLayouts == 1 then
		local opponentIx = match.bracketData.lowerEdges[1].opponentIx
...
function BracketDisplay.NodeLowerConnectors(props)
	...
	for ix = math.ceil(#bracketData.lowerEdges / 2), 1, -1 do
		...
		if not jointIxs[lowerEdge.opponentIx] then
			jointIxAbove = jointIxAbove + 1
			jointIxs[lowerEdge.opponentIx] = jointIxAbove
		end
	end
	...
	for ix = math.floor(#bracketData.lowerEdges / 2) + 1, #bracketData.lowerEdges, 1 do
		...
		if not jointIxs[lowerEdge.opponentIx] then
			...
			jointIxs[lowerEdge.opponentIx] = jointIxBelow
		end
	end
	...
	for _, lowerEdge in ipairs(bracketData.lowerEdges) do
		local lowerLayout = lowerLayouts[lowerEdge.lowerMatchIx]
		lowerConnectorsNode:node(
			BracketDisplay.NodeConnector({
				jointLeft = (config.roundHorizontalMargin - 2) * jointIxs[lowerEdge.opponentIx] / (jointCount + 1),
				leftTop = layout.lowerNodeMarginTop + heightSums[lowerEdge.lowerMatchIx] + lowerLayout.mid,
				lineWidth = config.lineWidth,
				rightTop = layout.matchMarginTop + ((lowerEdge.opponentIx - 1) + 0.5) * config.opponentHeight,
			})
		)
	end
...
function BracketDisplay.NodeConnector(props)
	...
	if props.leftTop == props.rightTop then
		...
		local leftNode = html.create('div'):addClass('brkts-line')
			...
			:css('right', props.joinRight and props.jointRight - props.lineWidth / 2 .. 'px')
			:css('left', '0')
			:css('top', props.leftTop - props.lineWidth / 2 .. 'px')
