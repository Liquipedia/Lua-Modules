...
function MatchGroupInput.readMatchlist(bracketid, args, matchBuilder)
	...
	for matchIndex = 1, 5000 do
		...
		match = require('Module:Brkts/WikiSpecific').processMatch(mw.getCurrentFrame(), match)
...
function MatchGroupInput.readBracket(bracketid, args, matchBuilder)
	...
	local bracketData = MatchGroupInput._getBracketData(templateid, bracketid)
	...
	for i = 1, k do
		...
		if match ~= nil then
			...
			MatchGroupInput._validateMatchBracketData(matchid, bd)
			...
			if winnerTo ~= nil then
				...
				bd['winnerto'] = winnerToMatch .. MatchGroupInput._convertMatchIdentifier(winnerTo)
			end
			...
			if loserTo ~= nil then
				...
				bd['loserto'] = loserToMatch .. MatchGroupInput._convertMatchIdentifier(loserTo)
			end
...
function MatchGroupInput._getBracketData(templateid, bracketid)
...
function MatchGroupInput._validateMatchBracketData(matchid, data)
	if data == nil then
...
function MatchGroupInput._convertMatchIdentifier(identifier)
