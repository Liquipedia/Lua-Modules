...
function p.getResultTypeAndWinner(data, indexedScores)
	...
	elseif Logic.readBool(data.finished) then
		if p.placementCheckDraw(indexedScores) then
			...
			indexedScores = p.setPlacement(indexedScores, data.winner, 'draw')
...
function p.setPlacement(opponents, winner, specialType, finished)
	...
	elseif specialType == 'default' then
		...
		local counter = 0
		for scoreIndex, opp in Table.iter.spairs(opponents, p.placementSortFunction) do
