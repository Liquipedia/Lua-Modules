...
module is specific to the Warcraft wiki.
]]
local WarcraftMatchGroupInput = {}

-- called from Module:MatchGroup
function WarcraftMatchGroupInput.processMatch(_, match)
	Table.mergeInto(
		match,
		WarcraftMatchGroupInput.readDate(match)
	)
	match = WarcraftMatchGroupInput.getTournamentVars(match)
	if match.ffa == 'true' then
		match = getWarcraftFfaInputModule().adjustData(match)
	else
		match = WarcraftMatchGroupInput.adjustData(match)
	end
	match = WarcraftMatchGroupInput.checkFinished(match)
	match = WarcraftMatchGroupInput.getVodStuff(match)
	match = WarcraftMatchGroupInput.getLinks(match)
	match = WarcraftMatchGroupInput.getExtraData(match)

	return match
end

function WarcraftMatchGroupInput.readDate(matchArgs)
...
function WarcraftMatchGroupInput.checkFinished(match)
...
function WarcraftMatchGroupInput.getTournamentVars(match)
...
function WarcraftMatchGroupInput.getVodStuff(match)
...
function WarcraftMatchGroupInput.getLinks(match)
	match.links = {
...
function WarcraftMatchGroupInput.getExtraData(match)
	if match.ffa == 'true' then
		match.extradata = getWarcraftFfaInputModule().getExtraData(match)
	else
		match.extradata = {
			...
			veto1 = WarcraftMatchGroupInput.getVetoMap(match.veto1),
			veto2 = WarcraftMatchGroupInput.getVetoMap(match.veto2),
			veto3 = WarcraftMatchGroupInput.getVetoMap(match.veto3),
			veto4 = WarcraftMatchGroupInput.getVetoMap(match.veto4),
			veto5 = WarcraftMatchGroupInput.getVetoMap(match.veto5),
			veto6 = WarcraftMatchGroupInput.getVetoMap(match.veto6),
			...
			subGroup1header = WarcraftMatchGroupInput.getSubGroupHeader(1, match),
			subGroup2header = WarcraftMatchGroupInput.getSubGroupHeader(2, match),
			subGroup3header = WarcraftMatchGroupInput.getSubGroupHeader(3, match),
			subGroup4header = WarcraftMatchGroupInput.getSubGroupHeader(4, match),
			subGroup5header = WarcraftMatchGroupInput.getSubGroupHeader(5, match),
			subGroup6header = WarcraftMatchGroupInput.getSubGroupHeader(6, match),
			subGroup7header = WarcraftMatchGroupInput.getSubGroupHeader(7, match),
			subGroup8header = WarcraftMatchGroupInput.getSubGroupHeader(8, match),
			subGroup9header = WarcraftMatchGroupInput.getSubGroupHeader(9, match),
			headtohead = match.headtohead,
			ffa = 'false',
			hero1 = WarcraftMatchGroupInput.getHeroes(match.hero1),
			hero2 = WarcraftMatchGroupInput.getHeroes(match.hero2),
			hero3 = WarcraftMatchGroupInput.getHeroes(match.hero3),
			hero4 = WarcraftMatchGroupInput.getHeroes(match.hero4),
			hero5 = WarcraftMatchGroupInput.getHeroes(match.hero5),
			hero6 = WarcraftMatchGroupInput.getHeroes(match.hero6),
			hero7 = WarcraftMatchGroupInput.getHeroes(match.hero7),
			hero8 = WarcraftMatchGroupInput.getHeroes(match.hero8),
		}
	end
...
function WarcraftMatchGroupInput.getVetoMap(map)
...
function WarcraftMatchGroupInput.adjustData(match)
	...
	if string.find(match.mode, 'team') then
		match = WarcraftMatchGroupInput.SubMatchStructure(match)
	end

	match = WarcraftMatchGroupInput.MatchWinnerProcessing(match)

	--Bracket Contest Handling
	if match.contest and tostring(match.contest.finished) == '1' then
		match = WarcraftMatchGroupInput.processContest(match)
	end
...
function WarcraftMatchGroupInput.MatchWinnerProcessing(match)
...
function WarcraftMatchGroupInput.SubMatchStructure(match)
	local SubMatches = {}
	local j = 0
...
function WarcraftMatchGroupInput.OpponentInput(match)
	for opponentIndex = 1, MAX_NUM_OPPONENTS do
		if not Logic.isEmpty(match['opponent' .. opponentIndex]) then
			...
			elseif match['opponent' .. opponentIndex]['type'] == 'quad' then
				match['opponent' .. opponentIndex] =
					WarcraftMatchGroupInput.ProcessOpponentInput(match['opponent' .. opponentIndex], 4)
			elseif match['opponent' .. opponentIndex]['type'] == 'team' then
				match['opponent' .. opponentIndex] =
					WarcraftMatchGroupInput.ProcessTeamOpponentInput(match['opponent' .. opponentIndex], match.date)
			elseif match['opponent' .. opponentIndex]['type'] == 'literal' then
				match['opponent' .. opponentIndex] =
					WarcraftMatchGroupInput.ProcessLiteralOpponentInput(match['opponent' .. opponentIndex])
			else
...
function WarcraftMatchGroupInput.ProcessSoloOpponentInput(opp)
...
function WarcraftMatchGroupInput.ProcessOpponentInput(opp, playernumber)
	local name = ''
...
function WarcraftMatchGroupInput.ProcessLiteralOpponentInput(opp)
...
function WarcraftMatchGroupInput.getPlayersLegacy(playerData)
	local players = {}
...
function WarcraftMatchGroupInput.getPlayers(teamName)
	local players = {}
	for playerIndex = 1, MAX_NUM_PLAYERS do
...
function WarcraftMatchGroupInput.ProcessTeamOpponentInput(opp, date)
	...
	if customTeam then
		...
		name, icon, opp.template = WarcraftMatchGroupInput.processTeamTemplateInput(opp.template, date)
	end
	name = mw.ext.TeamLiquidIntegration.resolve_redirect(name or '')
	local players = WarcraftMatchGroupInput.getPlayers(name)
	if Logic.isEmpty(players) then
		players = WarcraftMatchGroupInput.getPlayersLegacy(opp.players)
	end
...
function WarcraftMatchGroupInput.processTeamTemplateInput(template, date)
	local icon, name
...
function WarcraftMatchGroupInput.MapInput(match, i, subgroup)
	...
	match['map' .. i] = WarcraftMatchGroupInput.MapWinnerProcessing(match['map' .. i])

	--get participants data for the map + get map mode + winnerrace and loserrace
	--(w/l race stuff only for 1v1 maps)
	match['map' .. i] = WarcraftMatchGroupInput.ProcessPlayerMapData(match['map' .. i], match, 2)
...
function WarcraftMatchGroupInput.MapWinnerProcessing(map)
	map.scores = {}
	...
	if manual_scores then
		for scoreIndex, _ in Table.iter.spairs(indexedScores, WarcraftMatchGroupInput.placementSortFunction) do
...
function WarcraftMatchGroupInput.ProcessPlayerMapData(map, match, OppNumber)
	local participants = {}
	local map_mode = ''
	local raceOP = {}
	local PL = {}
	...
	for i = 1, OppNumber do
		...
		if match['opponent' .. i]['type'] == 'team' then
			...
			if number == 2 and map['opponent' .. i .. 'duoSpecial'] == 'true' then
				number = '2S'
...
function WarcraftMatchGroupInput.placementSortFunction(table, key1, key2)
