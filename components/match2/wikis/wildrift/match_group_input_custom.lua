...
local STATUS_DRAW = 'D'
local STATUSDEFAULT_WIN = 'W'
local STATUS_FORFEIT = 'FF'
local STATUS_DISQUALIFIED = 'DQ'
local STATUSDEFAULT_LOSS = 'L'
...
function CustomMatchGroupInput.placementCheckWL(tbl)
	return Table.any(tbl, function (_, scoreinfo) return scoreinfo.status == STATUSDEFAULT_LOSS end)
end
...
function CustomMatchGroupInput.getDefaultWinner(tbl)
	for index, scoreInfo in pairs(tbl) do
		if scoreInfo.status == STATUSDEFAULT_WIN then
			return index
		end
	end
	return NO_WINNER
...
function matchFunctions.getOpponents(match)
	...
	if Logic.isNumeric(match.walkover) then
		...
		opponents = matchFunctions._makeAllOpponentsLoseByWalkover(opponents, STATUSDEFAULT_LOSS)
		opponents[winnerIndex].status = STATUSDEFAULT_WIN
		match.finished = true
