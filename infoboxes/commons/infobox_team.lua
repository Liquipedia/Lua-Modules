...
function Team:createInfobox(frame)
	...
	infobox :name(args.name)
			...
			:fcell(Cell:new('Location'):options({})
				:content(Team:_createLocation(args.location), Team:_createLocation(args.location2)):make())
	...
	Team:addCustomHeadersAndCells(infobox, args)
...
function Team:addCustomHeadersAndCells(infobox, args)
    return infobox
end
