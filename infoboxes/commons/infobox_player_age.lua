...
function dateParams(output, birthfields, deathfields, DateBirth, DateDeath, DateCurrent)
		...
		if DateBirth then
	  	...
	  	if DateDeath then
			...
			output["death_year"] = DateDeath:text('%Y')
	  		output["death_display"] = result .. (resultAge and '<span class="noprint"> (aged&nbsp;' .. resultAge .. ')</span>' or '')
	  	else
			output["death_display"] = message('Need valid year, month, day (YYYY-MM-DD)')
			return output
	  	end
