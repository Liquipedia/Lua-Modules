...
function Locale.formatLocations(args)
	...
	local locations = Array.mapIndexes(function(index)
		...
		if not location.region and location.country then
			...
			location.region = String.nilIfEmpty(Region.run{region = location.country, onlyRegion = true} --[[@as string]])
			...
			if location.region then
				...
				location.region = String.nilIfEmpty(Region.run{country = location.country, onlyRegion = true} --[[@as string]])
			end

		elseif location.region then
			location.region = String.nilIfEmpty(Region.run{region = location.region, onlyRegion = true} --[[@as string]])
		end
