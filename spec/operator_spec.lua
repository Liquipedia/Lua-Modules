...
describe('operator', function()
	...
	describe('property', function()
		it('check', function()
			...
			assert.are_same({{c = 'd1'}, {c = 'd2'}}, Array.map(foo, Operator.property('a.b')))
			assert.are_same({'d1', 'd2'}, Array.map(foo, Operator.property('a.b.c')))
			assert.are_same({'f1', 'f2'}, Array.map(foo, Operator.property('a.e')))
			assert.are_same({'s1', 's2'}, Array.map(foo, Operator.property('a.1')))
			assert.are_same({'t1', 't2'}, Array.map(foo, Operator.property(1)))
			assert.are_same({'t1', 't2'}, Array.map(foo, Operator.property('1')))
			assert.are_same({'q1', 'q2'}, Array.map(foo, Operator.property('5')))
			assert.are_same({'q1', 'q2'}, Array.map(foo, Operator.property(5)))
			...
			-- intended type mismatch
			---@diagnostic disable-next-line: param-type-mismatch
			assert.error(Operator.property(foo))
			-- intended type mismatch
			---@diagnostic disable-next-line: param-type-mismatch
			assert.error(Operator.property(nil))
		end)
	end)
