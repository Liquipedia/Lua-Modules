...
insulate('LeagueIcon.display', function()
	it('should return filler icon when no icons are provided', function()
		...
		assert.are.equal('<span class="league-icon-small-image">[[File:Logo filler event.png|link=]]</span>', result)
	end)

	it('should return iconDark when only iconDark is provided', function()
		local args = {iconDark = 'DarkIcon.png'}
		local result = LeagueIcon.display(args)
		assert.are.equal('<span class="league-icon-small-image">[[File:DarkIcon.png|link=||50x50px]]</span>[[Category:Pages with only icondark]]', result)
	end)
	...
	it('should return both icons when both icon and iconDark are provided', function()
		...
		local result = LeagueIcon.display(args)
		assert.are.equal('<span class="league-icon-small-image lightmode">[[File:LightIcon.png|link=||50x50px]]</span><span class="league-icon-small-image darkmode">[[File:DarkIcon.png|link=||50x50px]]</span>', result)
	end)
...
insulate('LeagueIcon._make', function()
	it('should return single icon when icon and iconDark are identical', function()
		...
		assert.are.equal('<span class="league-icon-small-image">[[File:Icon.png|link=link|name|50x50px]]</span>', result)
	end)
	...
	it('should return both icons when icon and iconDark are different', function()
		...
		assert.are.equal('<span class="league-icon-small-image lightmode">[[File:LightIcon.png|link=link|name|50x50px]]</span><span class="league-icon-small-image darkmode">[[File:DarkIcon.png|link=link|name|50x50px]]</span>', result)
	end)
end)
...
insulate('LeagueIcon.getIconFromTemplate', function()
	...
	it('should extract icons from template string', function()
		...
		local icon, iconDark, trackingCategory = LeagueIcon.getIconFromTemplate({stringOfExpandedTemplate = templateString})
